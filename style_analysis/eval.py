...
```
{
  ...
  "method": {
    "karas",  # other options: "zlatkova", "nath", "pnb", "lof", "ac"
    "hyperparams": {
...
if method:
    method.train(config["datasets"]["train"])
    method.analyse_documents(config["datasets"])
...
results = model.analyse_documents(config["datasets"]["test"])
